...
class Selayer(nn.Module):
    ...
    def __init__(self, inplanes):
        ...
        self.conv1 = nn.Conv2d(inplanes,
                               ...
                               kernel_size=1,
                               stride=1)
        self.conv2 = nn.Conv2d(int(inplanes / 16),
                               inplanes,
                               kernel_size=1,
                               stride=1)
...
class baseline_HF_LF_SMAB_ConvNeXt(nn.Module):
    r""" ConvNeXt
        A PyTorch impl of : `A ConvNet for the 2020s`  -
          https://arxiv.org/pdf/2201.03545.pdf
...
@register_model
def baseline_HF_LF_SMAB_convnext_tiny(pretrained=False,
                                      in_22k=False,
                                      **kwargs):
    model = baseline_HF_LF_SMAB_ConvNeXt(depths=[3, 3, 9, 3],
                                         dims=[48, 96, 240, 384],
                                         **kwargs)
    if pretrained:
...
def convnext_tiny(pretrained=False, in_22k=False, **kwargs):
    model = ConvNeXt(depths=[3, 3, 9, 3], dims=[48, 96, 240, 384], **kwargs)
    if pretrained:
        url = model_urls['convnext_tiny_22k'] if in_22k else model_urls[
            'convnext_tiny_1k']
        checkpoint = torch.hub.load_state_dict_from_url(url=url,
