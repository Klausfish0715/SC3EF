    ...
    def add_image_summary(
        ...
        if self.total_steps % self.summary_freq == 0:
            img_concat = torch.cat(
                (img1[0].detach().cpu(), img2[0].detach().cpu()), dim=-1)
            img_concat = img_concat.type(
                torch.uint8)  # convert to uint8 to visualize in tensorboard

            # flow_pred = flow_tensor_to_image(flow_preds[-1])
            # forward_flow_gt = flow_tensor_to_image(flow_gt)
            flow_concat = torch.cat((flow_preds[-1], flow_gt), dim=1)

            # self.summary_writer.add_image(mode + '/img_pred_gt', concat, self.total_steps)
            self.summary_writer.add_image(mode + '/img_pred_gt', flow_concat,
                                          self.total_steps)
